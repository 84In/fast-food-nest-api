...
export class SeedService {
  constructor(
    ...
    @InjectModel(ProductIngrediant)
    private readonly productIngredientModel: typeof ProductIngrediant,
  ...
  private async seedUsers(transaction?: Transaction) {
    ...
    return await this.userModel.bulkCreate(userWithHashedPassword as any, {
      transaction,
    });
  }
  ...
  private async seedProducts(transaction?: Transaction) {
    ...
    return await this.productModel.bulkCreate(allProducts as any, {
      transaction,
    });
  }

  private async seedProductVariants(transaction?: Transaction) {
    return await this.productVariantModel.bulkCreate(productVariants as any, {
      transaction,
    });
  }

  private async seedProductIngredients(transaction?: Transaction) {
    return await this.productIngredientModel.bulkCreate(
